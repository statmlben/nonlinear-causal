...
for i in range(100):
	...
	beta0 = 1.
	Z, X, y = sim(n, p, theta0, beta0, case='linear')
...
## sim data to for categorical data for SIR
import numpy as np
from sklearn.preprocessing import normalize
from sim_data import sim
n, p = 2000, 100
...
for i in range(n_sim):
	...
	theta0 = theta0 / np.sqrt(np.sum(theta0**2))
	beta0 = 1.
	Z, X, y = sim(n, p, theta0, beta0, case='linear', feat='cate')
	LD_Z, cor_ZX, cor_ZY = np.dot(Z.T, Z), np.dot(Z.T, X), np.dot(Z.T, y)
...
d = {'abs_beta': beta_LS+beta_LS_norm+beta_LS_SIR, 
	 'method': ['LS']*n_sim+['norm_LS']*n_sim+['SIR+LS']*n_sim}
