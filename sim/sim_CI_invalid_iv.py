...
from nl_causal.linear_reg import WLasso, SCAD, L0_IC, SCAD_IC
...
for case in ['linear', 'log', 'cube-root', 'inverse', 'piecewise_linear']:
	for beta0 in [.05]:
		...
		for i in range(n_sim):
			...
			LS = ts_models._2SLS(sparse_reg=reg_model)
			...
			# print('est beta based on 2SLS: %.3f; CI: %s; len: %.3f' 
			# 	%(LS.beta*y_scale, LS.CI*y_scale, (LS.CI[1] - LS.CI[0])*y_scale))
			...
			RT_LS = ts_models._2SLS(sparse_reg=reg_model)
			## Stage-1 fit theta
			...
			# print('est beta based on RT-2SLS: %.3f; CI: %s; len: %.3f' 
			# %(RT_LS.beta*y_scale, RT_LS.CI*y_scale, (RT_LS.CI[1] - RT_LS.CI[0])*y_scale))

			## solve by SIR+LS
			reg_model = L0_IC(fit_intercept=False, alphas=10**np.arange(-1,3,.3), 
					Ks=Ks, max_iter=10000, refit=False, find_best=False)
			echo = ts_models._2SIR(sparse_reg=reg_model)
			## Stage-1 fit theta
			echo.fit_theta(Z1, X1)
			## Stage-2 fit beta
			echo.fit_beta(LD_Z2, cov_ZY2, n2=n2)
			## generate CI for beta
			...
			# print('est beta based on 2SIR: %.3f; CI: %s; len: %.3f' 
			# 	%(echo.beta*y_scale, echo.CI*y_scale, (echo.CI[1] - echo.CI[0])*y_scale))
			# len_SIR.append(1.)
		...
		print('PT-2SLS: beta0: %.3f; CI coverage: %.3f; CI len: %.3f'
				%(beta0, cover_RT_LS, np.mean(len_RT_LS)))
		print('2SIR: beta0: %.3f; CI coverage: %.3f; CI len: %.3f'
				%(beta0, cover_SIR, np.mean(len_SIR)))